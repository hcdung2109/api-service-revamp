...
class Tool
{
	...
	{
		if ($from == "") {
			$from = "no-reply@mhb4xg5ldz.emailserver.net.vn";
		}

		$host = "cp01hn.emailserver.net.vn";
		$username = "no-reply@mhb4xg5ldz.emailserver.net.vn"; // no-reply@vidu.vn
		$password = "aicoons@123";
	...
	public function image_resize($file, $max_width, $max_height, $save_file)
	{
		...
		// //  var_dump(getimagesize(ABSPATH . "assets/images/product.png"));
		// // exit;
		...
		switch ($image_type) {
			case 1:
				imagegif($tmp, $save_file);
				break;
			case 2:
				imagejpeg($tmp, $save_file, 100);
				break; // best quality
			case 3:
				imagepng($tmp, $save_file, 0);
	...
	{
		try {
			$ext = pathinfo(DOC_PATH . $file, PATHINFO_EXTENSION);
