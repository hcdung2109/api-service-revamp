...
use Closure;
use Digisource\Core\Traits\Cacheable;
use Illuminate\Contracts\Container\Container;
use Digisource\Core\Contracts\CacheableContract;
...
abstract class BaseRepository implements RepositoryContract, CacheableContract
{
    ...
    {
        $skipUri = $this->getContainer('config')->get($this->module.'.repository.cache.skip_uri');
    ...
    {
        $model = $this->getContainer('config')->get($this->module.'.repository.models');
        $className = static::class;
        // remove version number
        $className = preg_replace('/\\\V[0-9]/', '', $className);
        return $this->model ?: str_replace(['Repositories', 'Repository'], [$model, ''], $className);
    }
